...
parser.add_argument('--std', default=1e-3, type=float)
...
parser.add_argument('--logdir', default='logs/gld_const', type=str)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(args.maxiter):
            ...
            if epoch % args.frequence == 0:
                # eval matrix
                noise = np.random.normal(0, args.std, size=[model.n_weights])
